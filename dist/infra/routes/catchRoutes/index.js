...
const catchRoutes = (data) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const pokeball = yield prismaClient.item.findFirst({
        where: {
            ownerId: player.id,
            baseItem: {
                name: ballName,
    ...
    yield prismaClient.item.updateMany({
        where: {
            id: pokeball.id,
        },
        data: {
    ...
    console.log(`${player.name} ${catchRate > random ? 'catches' : 'fails to catch'} #${pokemon.id} ${pokemon.isShiny ? 'SHINY' : ''} ${pokemon.baseData.name}. BALL: ${ballName}, CHANCE: ${catchRate}`);
    if (catchRate > random) {
