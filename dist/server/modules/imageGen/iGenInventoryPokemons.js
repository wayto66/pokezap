...
exports.iGenInventoryPokemons = void 0;
const canvas_1 = require("canvas");
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const logger_1 = require("../../../infra/logger");
const fileHelper_1 = require("../../../server/helpers/fileHelper");
const pokemonTypes_1 = require("../../constants/pokemonTypes");
const talentIdMap_1 = require("../../constants/talentIdMap");
const getHoursDifference_1 = require("../../helpers/getHoursDifference");
const loadOrSaveImageFromCache_1 = require("../../helpers/loadOrSaveImageFromCache");
const iGenInventoryPokemons = (data) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    const { pokemons } = data;
    ...
    for (const type of pokemonTypes_1.pokemonTypes) {
        talentImageMap.set(type, yield (0, loadOrSaveImageFromCache_1.loadOrSaveImageFromCache)('./src/assets/sprites/UI/types/circle/' + type + '.png'));
    }
    let j = 0;
    let k = 0;
    for (let i = 0; i < 20; i++) {
        ...
        if (!spriteUrl)
            return;
        const sprite = yield (0, loadOrSaveImageFromCache_1.loadOrSaveImageFromCache)(spriteUrl);
        ...
        if (pokemons[i].heldItem) {
            ...
            const heldItemSprite = yield (0, loadOrSaveImageFromCache_1.loadOrSaveImageFromCache)(spUrl);
        ...
        if (pokemons[i].isAdult) {
            ...
            const type2Image = talentImageMap.get((_a = pokemons[i].baseData.type2Name) !== null && _a !== void 0 ? _a : pokemons[i].baseData.type1Name);
            if (!type2Image)
                continue;
            ...
            for (let i2 = 0; i2 < 3; i2++) {
                for (let j = 0; j < 3; j++) {
                    ...
                    const talentName = talentIdMap_1.talentIdMap.get(pokemons[i][talentIndex]);
                    if (!talentName)
                        continue;
            ...
            ctx.fillStyle = 'white';
            ctx.textAlign = 'end';
            ctx.fillText((24 - (0, getHoursDifference_1.getHoursDifference)(pokemons[i].createdAt, new Date())).toFixed(2) + ' h', x + 90, y + 35);
...
exports.iGenInventoryPokemons = iGenInventoryPokemons;
//# sourceMappingURL=iGenInventoryPokemons.js.map