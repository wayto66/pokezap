...
const tournamentDuelAccept = data =>
  __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (typeof sessionId !== 'number') throw new AppErrors_1.TypeMissmatchError(sessionIdString, 'number')
    const player2 = yield src_1.default.player.findFirst({
      where: {
        phone: data.playerPhone,
      },
    })
    if (!player2) throw new AppErrors_1.PlayerNotFoundError(data.playerPhone)
    ...
    if (!session || session.isFinished) throw new AppErrors_1.SessionIdNotFoundError(sessionId)
    if (
    ...
    if (session.invitedId !== player2.id) throw new AppErrors_1.SendEmptyMessageError()
    ...
    if (!winnerId) throw new AppErrors_1.UnexpectedError('duelo')
    if (!loserId) throw new AppErrors_1.UnexpectedError('duelo')
    if (isNaN(winnerId)) throw new AppErrors_1.TypeMissmatchError(winnerId.toString(), 'number')
    if (isNaN(loserId)) throw new AppErrors_1.TypeMissmatchError(loserId.toString(), 'number')
    ...
    if (!winner) throw new AppErrors_1.PlayerNotFoundError(winnerId.toString())
    if (!loser) throw new AppErrors_1.PlayerNotFoundError(loserId.toString())
    ...
    const updatedLoserPlayer = yield src_1.default.player
      ...
      .catch(e => logger_1.logger.error(e))
    if (!updatedLoserPlayer) throw new AppErrors_1.CouldNotUpdatePlayerError('id', loser.id)
    if (!updatedWinnerPlayer) throw new AppErrors_1.CouldNotUpdatePlayerError('id', winner.id)
    ...
    if (!loserPokemon0) throw new AppErrors_1.PokemonNotFoundError(duel.loserTeam[0].id)
    if (!winnerPokemon0) throw new AppErrors_1.PokemonNotFoundError(duel.winnerTeam[0].id)
    if (!loserPokemon1) throw new AppErrors_1.PokemonNotFoundError(duel.loserTeam[1].id)
    if (!winnerPokemon1) throw new AppErrors_1.PokemonNotFoundError(duel.winnerTeam[1].id)
    ...
    const winnerLevelUpMessage0 = (
      handleWinExp0 === null || handleWinExp0 === void 0 ? void 0 : handleWinExp0.leveledUp
    )
      ? `*${winnerPokemon0.baseData.name}* subiu para o nível ${handleWinExp0.pokemon.level}!`
      : ''
    const loserLevelUpMessage0 = (
      handleLoseExp0 === null || handleLoseExp0 === void 0 ? void 0 : handleLoseExp0.leveledUp
    )
      ? `*${loserPokemon0.baseData.name}* subiu para o nível ${handleLoseExp0.pokemon.level}!`
      : ''
    const winnerLevelUpMessage1 = (
      handleWinExp1 === null || handleWinExp1 === void 0 ? void 0 : handleWinExp1.leveledUp
    )
      ? `*${winnerPokemon1.baseData.name}* subiu para o nível ${handleWinExp1.pokemon.level}!`
      : ''
    const loserLevelUpMessage1 = (
      handleLoseExp1 === null || handleLoseExp1 === void 0 ? void 0 : handleLoseExp1.leveledUp
    )
...
${loserLevelUpMessage1}`
    yield src_1.default.tournament.update({
      where: {
        id: tournamentId,
      },
      data: {
