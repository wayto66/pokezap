...
exports.verifyTalentPermission = exports.duelNXN = void 0;
const iGenDuel2X1Rounds_1 = require("../../../../../image-generator/src/iGenDuel2X1Rounds");
const iGenDuel3X1Rounds_1 = require("../../../../../image-generator/src/iGenDuel3X1Rounds");
const iGenDuel3X2Rounds_1 = require("../../../../../image-generator/src/iGenDuel3X2Rounds");
const iGenDuel3X3Rounds_1 = require("../../../../../image-generator/src/iGenDuel3X3Rounds");
const iGenDuel3X4Rounds_1 = require("../../../../../image-generator/src/iGenDuel3X4Rounds");
const iGenDuelRound_1 = require("../../../../../image-generator/src/iGenDuelRound");
const iGenDuelX2Rounds_1 = require("../../../../../image-generator/src/iGenDuelX2Rounds");
...
const talentPowerBonusMap_1 = require("../../constants/talentPowerBonusMap");
const typeHeldItemMap_1 = require("../../constants/typeHeldItemMap");
...
const duelNXN = (data) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    while (duelFinished === false) {
        ...
        const getCurrentSkillForDamageRole = (attacker, enemies) => {
            ...
            for (const [skill, skillMap] of attacker.skillMap.damageSkills) {
                if (skill.pp <= 0)
                    console.log(`no more pp for ${skill.name}`);
        ...
        const dealDamage = (attacker, enemies) => {
            ...
            console.log(`${attacker.id}-${attacker.name} uses ${currentSkillData.skill.name} in ${currentSkillData.targets.map(t => t.name)}`);
        ...
        const support = (pokemon, allies) => __awaiter(void 0, void 0, void 0, function* () {
            ...
            currentSkillData.skill.pp -= 1;
            console.log(`${pokemon.id}-${pokemon.name} uses ${currentSkillData.skill.name} in ${currentSkillData.targets.map(t => t.name)}`);
        ...
        const getCurrentSkillForTankerRole = (pokemon, allies, enemies) => {
            ...
            if (skill.target === 'all-oponents' && skill.category.includes('damage'))
                targets = enemies;
            if (targets.length === 0) {
                logger_1.logger.error('no targets for: ' + skill.target);
                return undefined;
            }
            return {
                skill,
        ...
        const tank = (pokemon, allies, enemies) => __awaiter(void 0, void 0, void 0, function* () {
            ...
            if (currentSkillData.skill.category === 'damage+heal') {
                for (const target of currentSkillData.targets) {
                    const pwrWithADRatio = (((_k = pokemon.currentSkillPower) !== null && _k !== void 0 ? _k : 0) * adRatio(currentSkillData.skill, pokemon, target)) / 50;
                    const pwrDividedByTargets = pwrWithADRatio / currentSkillData.targets.length;
                    if (!target.block) {
                        target.hp -= pwrDividedByTargets * (0.9 + Math.random() * 0.2) * (1 - target.damageResistance);
                        pokemon.hp +=
        ...
        const handleRoundActions = (_l) => __awaiter(void 0, [_l], void 0, function* ({ poke, forceAttack }) {
            if (poke.hp <= 0)
                return;
        ...
        console.log((_f = leftTeamRoundData[0]) === null || _f === void 0 ? void 0 : _f.hp, ((_g = leftTeamRoundData[1]) === null || _g === void 0 ? void 0 : _g.hp) + ' VS ' + ((_h = rightTeamRoundData[0]) === null || _h === void 0 ? void 0 : _h.hp), (_j = rightTeamRoundData[1]) === null || _j === void 0 ? void 0 : _j.hp);
        if (roundCount > 145) {
            if (data.forceWin) {
                ...
                throw new AppErrors_1.UnexpectedError('duel exceeded 145 rounds.');
            }
        }
    }
    ...
    const imageGenFunctionMap = new Map([
        ['3vs1', iGenDuel3X1Rounds_1.iGenDuel3X1Rounds],
        ['2vs1', iGenDuel2X1Rounds_1.iGenDuel2X1Rounds],
        ['2vs2', iGenDuelX2Rounds_1.iGenDuelX2Rounds],
        ['1vs1', iGenDuelRound_1.iGenDuelRound],
        ['3vs2', iGenDuel3X2Rounds_1.iGenDuel3X2Rounds],
        ['3vs3', iGenDuel3X3Rounds_1.iGenDuel3X3Rounds],
        ['3vs4', iGenDuel3X4Rounds_1.iGenDuel3X4Rounds],
    ]);
    const imageGen = imageGenFunctionMap.get(leftTeamData.length + 'vs' + rightTeamData.length);
    if (!imageGen)
        throw new AppErrors_1.UnexpectedError('Could not get imageGen function for: ' + leftTeamData.length + 'vs' + rightTeamData.length);
...
const getBestSkills = (_m) => __awaiter(void 0, [_m], void 0, function* ({ attacker, defenders }) {
    var _o;
    ...
    for (const skill of skills) {
        ...
        const talentBonus = (_o = talentPowerBonusMap_1.talentPowerBonusMap.get(talentCheck.count)) !== null && _o !== void 0 ? _o : 0;
...
const verifyTalentPermission = (poke, skill) => {
    const talents = [
        poke.talentId1,
        poke.talentId2,
        poke.talentId3,
...
const getHeldItemMultiplier = (pokemon, skill, xItemName) => {
    var _a;
    const heldName = (_a = pokemon.heldItem) === null || _a === void 0 ? void 0 : _a.baseItem.name;
    if (!heldName)
        return 1;
    if (heldName && plateTypeMap_1.plateTypeMap.get(heldName) === skill.typeName)
        return 1.07;
    if (heldName && heldName === skill.typeName + '-gem')
        return 1.14;
    if (heldName && typeHeldItemMap_1.typeHeldItemMap.get(skill.typeName) === heldName)
        return 1.2;
    if (heldName && heldName === xItemName)
        return 1.11;
    return 1;
};
