...
export const tournamentDuelAccept = async (data: TRouteParams) => {
...
${loserLevelUpMessage1}`
  ...
  setTimeout(() => {
    sendMessage(
      ...
      tournament?.gameRoom.phone ?? ''
    )
    tournamentDuel({ ...data, routeParams: ['pz', 'torneio', 'duel', tournamentIdString, ...pairs] })
  }, 10000)
  ...
  sendMessage(response, tournament?.gameRoom.phone ?? '')
}
