...
export const raidCreate = async (data: TRouteParams): Promise<IResponse> => {
  ...
  const checkRaid = await prismaClient.raid.findFirst({
    where: {
  ...
  const bossBaseData = await prismaClient.basePokemon.findFirst({
    where: {
      name: raidName,
    },
  })

  const enemiesBaseData = await prismaClient.basePokemon.findMany({
    where: {
      name: {
        in: raidData.enemies,
      },
  ...
  const lootData = await prismaClient.baseItem.findMany({
    where: {
      name: {
