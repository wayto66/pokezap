...
export const ContinuousDuel6x6 = async (data: TParams): Promise<TDuelNXNResponse | void> => {
  ...
  while (duelFinished === false) {
    ...
    const roundStart = (poke: RoundPokemonData) => {
      if (poke.hp <= 0) {
        ...
        if (poke.team === 'left') {
          ...
          if (nextPoke) pokemonsInDuelOrder.push(nextPoke)
    ...
    for (const poke of pokemonsInDuelOrder) {
      if (!poke) continue
      if (poke.team === 'left') leftTeamRoundData.push({ ...poke })
      else rightTeamRoundData.push({ ...poke })
    }
    ...
    if (roundCount > 360) {
      throw new UnexpectedError('duel exceeded 360 rounds.')
    }
  }
  ...
  const imageUrl = await iGenDuelX6Rounds({
    duelMap: duelMap,
    roundCount,
