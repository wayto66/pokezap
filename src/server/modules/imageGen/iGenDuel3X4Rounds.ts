...
export const iGenDuel3X4Rounds = async (data: TDuelRoundData): Promise<string> => {
  const filepath = await new Promise<string>(async resolve => {
    ...
    // load poke3 sprite
    const rightPokeSprite2 = await loadOrSaveImageFromCache(rightTeam[2].spriteUrl)
    // load poke4 sprite
    const rightPokeSprite3 = await loadOrSaveImageFromCache(rightTeam[3].spriteUrl)

    const skillFlagImagesMap = new Map<string, Image>([])

    for (const poke of [leftTeam, rightTeam].flat()) {
    ...
    const drawStillPart = (roundInfo: DuelNxNRoundData) => {
      ...
      canvas.write({
        ...
        positionY: 135,
        textAlign: 'start',
        text: rightTeam[2].level.toString(),
      })
      ...
      canvas.write({
        ...
        positionY: 5,
        textAlign: 'start',
        text: rightTeam[3].level.toString(),
      })
    ...
    for (let i = 0; i < data.roundCount * framesPerRound + 12; i++) {
      ...
      canvas.fillRect({
        ...rightHpDefaults,
        y: 107,
        w: Math.max(0, (roundInfo.rightTeamData[2].hp / roundInfo.rightTeamData[2].maxHp) * 125),
      })
      canvas.fillRect({
        ...rightHpDefaults,
        y: 155,
        w: Math.max(0, (roundInfo.rightTeamData[3].hp / roundInfo.rightTeamData[3].maxHp) * 125),
      })
      ...
      canvas.fillRect({ ...rightManaDefaults, y: 15, w: Math.max(0, (roundInfo.rightTeamData[0].mana / 100) * 175) })
      canvas.fillRect({ ...rightManaDefaults, y: 65, w: Math.max(0, (roundInfo.rightTeamData[1].mana / 100) * 175) })
      canvas.fillRect({ ...rightManaDefaults, y: 110, w: Math.max(0, (roundInfo.rightTeamData[2].mana / 100) * 175) })
      canvas.fillRect({ ...rightManaDefaults, y: 160, w: Math.max(0, (roundInfo.rightTeamData[3].mana / 100) * 175) })
      ...
      canvas.write({
        ...rightTeamSkillNameDefaults,
        positionY: 425,
        text: roundInfo.rightTeamData[2].currentSkillName ?? '',
      })
      canvas.write({
        ...rightTeamSkillNameDefaults,
        positionY: 475,
        text: roundInfo.rightTeamData[3].currentSkillName ?? '',
      })
