...
export const iGenRocketInvasion = async (data: TParams) => {
  ...
  const backgroundImage = await loadImage('./src/assets/sprites/UI/hud/rocket-invasion.png')
  ...
  await canvas2d.draw({
    ...
    positionY: 340,
    positionX: 170,
    image: await loadImage(pokemons[0]?.spriteUrl),
  })
  canvas2d.write({
    fillStyle: 'white',
    font: '11px Pokemon',
  ...
  await canvas2d.draw({
    ...
    positionY: 340,
    positionX: 260,
    image: await loadImage(pokemons[1]?.spriteUrl),
  })
  canvas2d.write({
    fillStyle: 'white',
    font: '11px Pokemon',
