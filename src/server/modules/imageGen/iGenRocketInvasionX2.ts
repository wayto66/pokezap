...
export const iGenRocketInvasionX2 = async (data: TParams) => {
  ...
  const backgroundImage = await loadImage('./src/assets/sprites/UI/hud/rocket_invasion_x2.png')
  ...
  await canvas2d.draw({
    ...
    positionX: 100,
    positionY: 100,
    image: await loadImage(rocketAvatarSpriteUrl),
  })
  ...
  await canvas2d.draw({
    ...
    positionY: 75,
    positionX: 50,
    image: await loadImage(pokemon1.spriteUrl),
  })
  ...
  await canvas2d.draw({
    ...
    positionY: 290,
    positionX: 50,
    image: await loadImage(pokemon2.spriteUrl),
  })
...
const getTalents = (pokemon: DuelPokemon) => {
