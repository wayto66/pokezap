...
export const iGenDuel3X1Rounds = async (data: TDuelRoundData): Promise<string> => {
  console.log('igen duel2x1 rounds')
  console.log({ data })
  const filepath = await new Promise<string>(async resolve => {
    const boss = data.boss
    const allyTeam = data.alliesTeam

    const backgroundUrl = './src/assets/sprites/UI/hud/duel_3x1_round.png'
    ...
    // load poke3 sprite
    const allyPokeSprite2 = await loadImage(allyTeam[2].spriteUrl)
    ...
    const drawStillPart = (roundInfo: BossInvasionRoundData) => {
      ...
      if (roundInfo.alliesTeamData[2].hp > 0)
        canvas.draw({
          height: 251,
          width: 251,
          positionX: -51,
    ...
    for (let i = 0; i < data.roundCount * framesPerRound + 12; i++) {
      ...
      if (!roundInfo) {
        console.log('no round info for: ' + round)
        continue
      }
      ...
      canvas.fillRect({
        ...
        x: 55,
        y: 107,
        w: Math.max(0, (roundInfo.alliesTeamData[2].hp / roundInfo.alliesTeamData[2].maxHp) * 125),
        h: 7,
      })
      ...
      canvas.fillRect({
        ...
        x: 5,
        y: 100,
        w: Math.max(0, (roundInfo.alliesTeamData[1].mana / 100) * 175),
        h: 3,
      })
      ...
      canvas.fillRect({
        ...
        x: 5,
        y: 145,
        w: Math.max(0, (roundInfo.alliesTeamData[2].mana / 100) * 175),
        h: 3,
      })
      ...
      canvas.write({
        ...
        positionX: 15,
        positionY: 475,
        text: roundInfo.alliesTeamData[2].currentSkillName,
      })
