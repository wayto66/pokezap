...
export const iGenDuel3X3Rounds = async (data: TDuelRoundData): Promise<string> => {
  const filepath = await new Promise<string>(async resolve => {
    ...
    for (let i = 0; i < data.roundCount * framesPerRound + 12; i++) {
      ...
      canvas.fillRect({ ...rightManaDefaults, y: 50, w: Math.max(0, (roundInfo.rightTeamData[0].mana / 100) * 175) })
      canvas.fillRect({ ...rightManaDefaults, y: 100, w: Math.max(0, (roundInfo.rightTeamData[1].mana / 100) * 175) })
      canvas.fillRect({ ...rightManaDefaults, y: 145, w: Math.max(0, (roundInfo.rightTeamData[2].mana / 100) * 175) })
